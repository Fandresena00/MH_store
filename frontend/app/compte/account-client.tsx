  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-10 lg:px-10 lg:py-14">
      <div className="flex items-center gap-4">
        <div className="flex h-14 w-14 items-center justify-center rounded-full text-lg font-semibold text-white" style={{ background: "var(--teal)" }}>
          HR
        </div>
        <div>
          <h1 className="font-display text-2xl">Bonjour, Hanta</h1>
      ...
      <div className="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-[220px_1fr]">
        <nav className="space-y-1">
          ...
          <button className="mt-4 flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium" style={{ color: "var(--error)" }}>
          ...
          {tab === "commandes" && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="font-display text-xl">Historique de commandes</h2>
                <Link href="/suivi-commande" className="link-underline text-sm font-medium" style={{ color: "var(--teal-deep)" }}>
                  Suivi détaillé
                </Link>
              </div>
              {orders.map((o) => (
                <div key={o.id} className="card-hairline flex flex-wrap items-center justify-between gap-3 p-5">
                  <div>
                  ...
                  <Badge variant="outline" className={statusClassName(o.status)}>{o.status}</Badge>
                  <span className="font-display text-base">{formatAr(o.total)}</span>
                  <Link href={`/suivi-commande?commande=${o.id}`} className="link-underline text-sm font-medium">
                    Détails
                  </Link>
                </div>
              ))}
          ...
          {tab === "adresses" && (
            <div className="space-y-4">
              ...
              <div className="card-hairline max-w-sm p-5">
                ...
                <button className="link-underline mt-3 text-sm font-medium">Modifier</button>
              </div>
              <button className="btn-primary px-6 py-3 text-sm font-semibold">+ Ajouter une adresse</button>
            </div>
          )}
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-5 sm:grid-cols-3">
                {products.slice(0, 3).map((p) => (
                  <Link key={p.slug} href={`/produit/${p.slug}`} className="card-hairline overflow-hidden">
                    <ProductMedia src={p.images[0]} alt={p.name} className="aspect-square" />
                    <div className="p-3">
                      <p className="text-sm font-medium">{p.name}</p>
                      <p className="text-sm" style={{ color: "var(--ink-soft)" }}>{formatAr(p.price)}</p>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {tab === "parametres" && (
            <div className="max-w-md space-y-5">
              ...
              <div>
                ...
                <input defaultValue="hanta.r@email.com" className="field-input" />
              </div>
              <div>
                <label className="field-label">Téléphone</label>
                <input defaultValue="+261 34 00 000 00" className="field-input" />
              </div>
              <button className="btn-primary px-6 py-3 text-sm font-semibold">Enregistrer</button>
            </div>
          )}
        </div>
      </div>
