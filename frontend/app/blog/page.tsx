...
export const metadata: Metadata = {
  title: "Le journal",
  description: "Reportages et carnets sur l'artisanat malgache et les mains qui le font vivre.",
};
  ...
  return (
    <div className="mx-auto max-w-[1200px] px-5 py-10 lg:px-10 lg:py-14">
      <div className="text-center">
        ...
        <h1 className="mt-2 font-display text-4xl">Matières, gestes et ateliers</h1>
      ...
      {/* --------------------------------------------------------------- FEATURED */}
      <Link href={`/blog/${first.slug}`} className="group mt-12 grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-10">
        <ProductMedia src={first.cover} alt={first.title} className="aspect-[4/3] rounded-xl" />
        <div className="flex flex-col justify-center">
          <p className="eyebrow">{first.category}</p>
          <h2 className="link-underline mt-2 font-display text-3xl leading-snug">{first.title}</h2>
      ...
      <div className="grid grid-cols-1 gap-8 sm:grid-cols-2">
        {rest.map((post) => (
          <Link key={post.slug} href={`/blog/${post.slug}`} className="group">
            ...
            <h3 className="link-underline mt-1.5 font-display text-xl leading-snug">{post.title}</h3>
