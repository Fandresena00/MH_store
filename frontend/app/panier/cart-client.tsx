...
import { RiSubtractLine, RiAddLine, RiCloseLine, RiArrowRightLine, RiShoppingBasket2Line } from "@remixicon/react";
  ...
  if (lines.length === 0) {
    return (
      <div className="mx-auto flex max-w-lg flex-col items-center px-5 py-28 text-center">
        ...
        <Link href="/boutique" className="btn-primary mt-8 px-8 py-3.5 text-sm font-semibold">
    ...
    <div className="mx-auto max-w-[1200px] px-5 py-10 lg:px-10 lg:py-14">
      ...
      <div className="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-[1fr_360px]">
        <div className="divide-y" style={{ borderColor: "var(--line)" }}>
          {lines.map((line) => (
            <div key={line.product.slug} className="flex gap-4 border-b py-6" style={{ borderColor: "var(--line)" }}>
              <Link href={`/produit/${line.product.slug}`} className="shrink-0">
                <ProductMedia src={line.product.images[0]} alt={line.product.name} className="h-28 w-28 rounded-lg" />
              </Link>
              <div className="flex flex-1 flex-col justify-between">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <Link href={`/produit/${line.product.slug}`} className="link-underline font-display text-lg">
                      {line.product.name}
                    </Link>
                ...
                <div className="flex items-end justify-between">
                  ...
                  <span className="font-display text-base">{formatAr(line.product.price * line.quantity)}</span>
                </div>
              </div>
            </div>
          ))}
            ...
            <div className="flex gap-4 overflow-x-auto pb-2">
              {products
                ...
                .map((p) => (
                  ...
                  >
                    <ProductMedia src={p.images[0]} alt={p.name} className="aspect-square w-full rounded-lg" />
                    <p className="mt-2 line-clamp-1 text-xs font-medium">{p.name}</p>
                    <p className="text-xs" style={{ color: "var(--ink-soft)" }}>{formatAr(p.price)}</p>
                  </button>
                ))}
            </div>
          </div>
